...
game:GetService("RunService").Heartbeat:Connect(function()
	local Loudness = Gears[1].Handle.Sound.PlaybackLoudness/20
	for i,Gear in pairs(Gears) do
		local Pos = CFrame.new(Character.HumanoidRootPart.Position)
		local AmountOffset = math.pi/#Gears*i*2
		local Topos = (Pos + Pos.LookVector * math.sin(tick()+AmountOffset)*(Loudness+1) + Pos.RightVector * math.cos(tick()+AmountOffset)*(Loudness+1))
		Gear.Handle.CFrame = CFrame.lookAt(Topos.Position, Character.HumanoidRootPart.Position)
